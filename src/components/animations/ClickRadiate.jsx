...
const ClickRadiate = ({
  ...
  useEffect(() => {
    ...
    const draw = (timestamp) => {
      ...
      particlesRef.current = particlesRef.current.filter((p) => {
        ...
        // Grow ray length each frame
        p.length += p.vlength;

        // Draw from midpoint to tip — tail lifts off origin
        // giving a travelling beam rather than a static spoke
  ...
  const handleClick = useCallback(
    (e) => {
      ...
      const newParticles = Array.from({ length: rayCount }, () => ({
        x: cx,
        y: cy,
